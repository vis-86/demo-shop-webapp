...
import { useStartWebApp } from "@/fetcher/user/client";

const enableInitWebApp = false
...
export const TgCartProvider = ({ children }: { children?: ReactNode | undefined }): ReactNode => {
  ...
  const { mutate } = useStartWebApp()
  const [initWebApp, setInitWebApp] = useState(false)

  useEffect(() => {
    if (!enableInitWebApp) return 
  ...
  }, [tg, tgScriptReady, initWebApp, mutate])
  ...
  }, [tgApp, tgScriptReady, tgEnabled, mutate])
